// File: app/api/tokens/search/route.ts
...
export async function GET(request: NextRequest) {
  ...
  try {
    // Check if query looks like a token address (base58, 32-44 characters)
    const isTokenAddress = /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(query.trim())
    
    if (isTokenAddress) {
      // For token addresses, get detailed token information
      try {
        ...
        if (response.ok) {
          ...
          // Get the first pool for liquidity and price data
          ...
          const detailedToken = {
            ...
            isDetailedView: true // Flag to show detailed view
          }
    ...
    // Fallback: fetch trending tokens and filter them
